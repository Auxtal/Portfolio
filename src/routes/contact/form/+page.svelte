    ...
    <form class="mt-6" method="POST" action="?/contact" use:enhance={contactSubmit}>
      ...
      <div class="form-item relative mt-2 mb-5 ">
        <textarea
          ...
          class="peer textarea my-2 block w-full !rounded-md bg-transparent px-4 pt-3 text-sm text-secondary shadow-sm outline outline-1 outline-secondary backdrop-blur-sm transition-all focus:outline-[2.5px] focus:outline-neutral {form
            ?.errors?.message
            ? '!outline-red-500'
            : ''}"
          rows="5"
